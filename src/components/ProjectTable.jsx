...
export default function ProjectTable({ title, data = [], loading }) {
        ...
        <table className="w-full table-auto text-sm text-left">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="text-center py-6">No projects found.</td>
              </tr>
            ) : (
              data.map((row, idx) => (
                <tr key={idx} className="border-t text-gray-700 font-medium">
                  <td className="px-4 py-3">{row.company || row.companyName}</td>
